...
# Run the crawler
go run scripts/chromedp_scraper/main.go
```
...
- **Data Age**: This crawler fetches HISTORICAL data from February 2022, NOT current draws
- **Purpose**: Useful for training prediction algorithms with more historical context
- **Current Data**: Your existing 8 draws (Dec 2025 - Jan 2026) are still the most recent
- **Speed**: Headless browser takes ~2-3 seconds per page (total ~3-4 minutes for 50 draws)
...
Edit `main.go` and increase the timeout:
...
Increase JavaScript wait time in `main.go`:
...
2. **Run predictor** with expanded dataset:
   ```bash
   ./bin/predictor -g POWER_6_55
   ```

3. **Run backtesting** with more data:
   ```bash
